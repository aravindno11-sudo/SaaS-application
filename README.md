...
# Install all deps at once (from root)
npm run install:all
...
# Edit backend/.env — set MONGODB_URI, JWT_SECRET, CORS_ORIGIN
...
### Testing the Payment Flow (Mock Mode)
1. Register a new user.
2. From the dashboard, click **"Upgrade to Pro"**.
3. On the Pricing page, choose **Stripe** or **Razorpay** with the toggle.
4. Click **"Upgrade Now"** → you'll land on the Mock Checkout page.
5. Fill in any test details and click **Pay**.
6. The Success page auto-upgrades your workspace to Pro.
...
| Step | Detail |
|---|---|
| New service | **Web Service** → connect your GitHub repo |
| Root dir | `backend` |
...
| `STRIPE_SECRET_KEY` | `sk_test_placeholder` (or real key) |
| `STRIPE_WEBHOOK_SECRET` | `whsec_placeholder` (or real secret) |
...
| Step | Detail |
|---|---|
| Import project | Connect GitHub repo in Vercel dashboard |
| Root dir | `frontend` |
...
> After setting env vars in Vercel, trigger a **Redeploy** for them to take effect.
...
- `authorizeWorkspace` middleware enforces RBAC per operation.
- Markdown export is streamed as a blob and gated to Pro workspaces.

---

## 🏭 Production Checklist

- [ ] Replace `STRIPE_SECRET_KEY` with a real Stripe live/test key
- [ ] Remove or guard the `/mock-upgrade` and `/mock-checkout` routes
- [ ] Set `JWT_SECRET` to a strong randomly-generated value (`node -e "console.log(require('crypto').randomBytes(64).toString('hex'))"`)
- [ ] Set `CORS_ORIGIN` to exact deployed frontend URL (no trailing slash)
- [ ] Add rate limiting to `/api/auth` endpoints
- [ ] Enable MongoDB Atlas IP allowlist for your Render service IPs
